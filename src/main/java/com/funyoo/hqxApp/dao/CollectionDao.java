...
public interface CollectionDao {
    ...
    @Select("SELECT * FROM user_news_collection_${id} WHERE id < #{num} ORDER BY id DESC LIMIT #{readNum}")
    ...
    @Select("SELECT * FROM user_news_collection_${id} ORDER BY id DESC LIMIT #{readNum}")
    public List<CollectionModel> getCollectionsLimit(@Param("id") Integer id, @Param("readNum") Integer OnceReadNum);

    @Select("SELECT * FROM user_news_collection_${id} ORDER BY id DESC")
    ...
    @Update("CREATE TABLE `user_news_collection_${id}` (\n" +
    ...
    @Insert("INSERT INTO user_news_collection_${uid} (aid, part, title, type, html_url, date) " +
            "VALUES " +
    ...
    @Select("SELECT id from user_news_collection_${uid} WHERE aid = #{aid} AND part = #{part}")
    Integer isCollected(@Param("uid") Integer uid, @Param("part") String part, @Param("aid") Integer artId) throws Throwable;

    @Delete("DELETE FROM user_news_collection_${uid} WHERE aid = #{aid} AND part = #{part}")
